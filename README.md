*Este projeto foi construído para o Senai Maracanã, como trabalho de conclusão de curso (TCC), referente ao curso de Tecnologia da informação (TI), baseado nesse Briefing.*

# Briefing — Empresa de Gestão de Processos Digitais

## Nome da Empresa: FlowPilot

### História:

A **FlowPilot** surgiu como uma consultoria voltada para **organização e padronização de
processos internos** em pequenas e médias empresas.
Com o avanço da tecnologia e a transformação digital, percebeu a necessidade de oferecer
soluções mais ágeis e acessíveis.
Por isso, evoluiu de uma consultoria tradicional para uma **plataforma digital de gestão de
processos**, que ajuda empresas a **automatizar tarefas, monitorar resultados e engajar
equipes.**

Agora, a marca está passando por um **reebranding**, buscando uma **nova identidade
visual** que traduza modernidade, tecnologia e facilidade de uso.

### Área de Atuação:

Gestão e automação de processos empresariais (Business Process Management – BPM)
com foco digital.

A empresa atua no desenvolvimento de ferramentas online (site e aplicativo) que auxiliam
gestores a mapear fluxos de trabalho, acompanhar tarefas e melhorar a produtividade das
equipes.

### Serviço / Produto:

  - A **FlowPilot** oferece uma **plataforma digital** (App e Site) que permite:
Criar fluxos de trabalho personalizados.

  - Acompanhar o andamento de tarefas em tempo real.
Receber relatórios automáticos de desempenho.

  - Engajar colaboradores por meio de **gamificação** (sistema de pontos,
conquistas e ranking).

O foco é **tornar a gestão de processos mais simples, visual e motivadora**.

### Público-Alvo:
  - **Pequenas e médias empresas** que desejam organizar e digitalizar seus
processos internos.

  - **Gestores e coordenadores** que buscam mais controle e eficiência nas
rotinas de trabalho.

  - **Equipes operacionais** que precisam de ferramentas simples para
acompanhar suas atividades.

Faixa etária média: 25 a 50 anos.
Perfil: Profissionais com rotina corporativa e interesse em soluções digitais práticas.

### Objetivo do Rebranding:

O rebranding tem como objetivo atualizar a imagem da FlowPilot para o novo momento
digital da marca.
A nova identidade deve refletir:

  - Inovação e tecnologia
  - Confiabilidade e clareza,
  - Agilidade e modernidade.
    
### Resumo:

A **FlowPilot** está em processo de transformação.
De uma consultoria tradicional, passa a ser uma **empresa digital**, moderna e interativa, que
conecta tecnologia e gestão de forma leve e envolvente.
O desafio é traduzir essa nova fase visualmente e digitalmente, por meio de um **rebranding
completo**.


### Agradecimentos:

Gostaria de agradecer aos instrutores e amigos que me ajudaram a desenvolver este projeto. Agradeço, em primeiro lugar, aos instrutores **Matheus Hermenegildo** e **Luciano Alonso**, assim como a **Nivaldo Batista**, **Gabriel Raimundo** e **Vicente Orsino**, que cederam espaço para trabalho (Fablab) e auxiliaram na resolução de problemas da página. Agradeço também aos amigos **Davi Vasconcellos**, **Vinícius Honório (Gaúcho)**, **Jonathan Mattos**, **Isaque Gomes** e **Gabriel da Silva (Panelinha)**, que me deram ideias de funcionalidades e desingn para adicionar ao site. Por fim, agradeço à minha equipe designada para o projeto — **Bruna Soares (Jogos)**, **Emilly Vicente (Multimídia)**, **Graziele da Silva (Multimídia)** e **Lucas de Morais (Design)** — pela ajuda no meu desenvolvimento pessoal e no desenvolvimento do projeto. **Muito obrigado a todos; Vocês são Incríveis**.

# FlowPilot — Resumo do Projeto (Funcionalidades)

Breve descrição
----------------
FlowPilot é uma interface frontend estática para gestão e monitoramento de processos. Oferece páginas institucionais, autenticação de usuários e um painel principal (dashboard) com componentes interativos e visualizações de dados.

Principais recursos
-------------------
- Autenticação: telas de `login` e `register`.
- Dashboard com cards, gráficos, lista de atividades e notificações (`pages/dashboard.html`).
- Páginas institucionais: `about`, `contact`, `help`, `terms`, `privacy`.
- Layout responsivo com estilos organizados em `css/`.

Estrutura do repositório
------------------------
- `index.html` — página inicial
- `pages/` — páginas secundárias, incluindo `dashboard.html`
- `css/` — folhas de estilo (`style.css`, `register.css`, `responsive.css`, etc.)
- `scripts/` — JavaScript do site (`login.js`, `register.js`, `notifications.js`, `auth-check.js`, `main.js`)
- `assets/images/` — imagens e recursos estáticos

Tecnologias
-----------
- HTML5
- CSS3
- JavaScript (ES6)
- React
------------------

**Esse projeto é apenas um protótipo, não utilizar com dados reais**
